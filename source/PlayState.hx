...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Youre Still Doing Great!', 0.2], //From 0% to 19%
		['Dont Worry!', 0.4], //From 20% to 39%
		['A little bit better', 0.5], //From 40% to 49%
		['Ok!', 0.6], //From 50% to 59%
		['Cool!', 0.69], //From 60% to 68%
		['Oh 0-0', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Rad!', 0.9], //From 80% to 89%
		['Sweet!', 1], //From 90% to 99%
		['Woah good job man!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'snowy': //Week 1
				...
				var stageFront:BGSprite = new BGSprite('snowy/Sky', -500, -50, 0.8, 0.8);
				stageFront.setGraphicSize(Std.int(stageFront.width * 0.76));
				stageFront.updateHitbox();
				add(stageFront);
				
				var stageLight:BGSprite = new BGSprite('snowy/mountains', -1000, 100, 0.9, 0.9);
				stageLight.setGraphicSize(Std.int(stageLight.width * 0.9));
				stageLight.updateHitbox();
				add(stageLight);

				var bg:BGSprite = new BGSprite('snowy/BG_snow', -700, 870, 1, 1);
				add(bg);

				var frozenlake:BGSprite = new BGSprite('snowy/frozen_lake', -1000, 750, 0.9, 0.9);
				frozenlake.setGraphicSize(Std.int(frozenlake.width * 0.9));
				frozenlake.updateHitbox();
				add(frozenlake);

				var stageCurtains:BGSprite = new BGSprite('snowy/FG_Snow', -400, 800, 1.3, 1.3);
